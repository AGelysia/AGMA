...
import dev.minecraftagent.standalone.ui.StandaloneCatalogScreen;
import dev.minecraftagent.standalone.ui.StandaloneCatalogService;
import dev.minecraftagent.standalone.ui.StandaloneSettingsScreen;
...
public final class StandaloneAssistantScreen extends Screen {
  ...
  private void handleRuntimeAction(RuntimePanelStatus.Action action) {
    switch (action) {
      case CONFIGURE ->
          minecraft.setScreen(
              new StandaloneSettingsScreen(catalog, runtime, tools, toolRouter, state));
  ...
  private void startRuntime() {
    ...
    var startup = runtime.start(toolRouter);
    rebuild();

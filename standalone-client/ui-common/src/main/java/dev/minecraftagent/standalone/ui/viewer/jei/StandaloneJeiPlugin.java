package dev.minecraftagent.standalone.ui.viewer.jei;
 ...
 * only when its own runtime is present.
 */
public class StandaloneJeiPlugin implements IModPlugin {
  private static final String ADAPTER_ID = "jei";

  private final ModMetadataSource metadataSource;

  protected StandaloneJeiPlugin(ModMetadataSource metadataSource) {
    this.metadataSource = java.util.Objects.requireNonNull(metadataSource, "metadataSource");
  }
  ...
  public void onRuntimeAvailable(IJeiRuntime runtime) {
    OptionalViewerRegistry.register(new JeiAdapter(metadataSource, runtime));
  }
  ...
  private static final class JeiAdapter implements CatalogAdapter {
    ...
    private JeiAdapter(ModMetadataSource metadataSource, IJeiRuntime runtime) {
    ...
    @Override
    public Probe probe() {
      var installed = metadataSource.installedVersion("jei").orElse(null);
    ...
    private <T> ProcessRecord toProcess(
        IRecipeCategory<T> category,
        T recipe,
        int recipeIndex,
    ...
    private List<ResourceRef> itemResources(

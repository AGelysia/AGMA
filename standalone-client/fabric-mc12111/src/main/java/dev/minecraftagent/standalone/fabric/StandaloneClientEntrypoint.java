...
public final class StandaloneClientEntrypoint implements ClientModInitializer {
  ...
  private static void registerPreviewHud() {
    HudElementRegistry.attachElementBefore(
        ...
        Identifier.fromNamespaceAndPath(MOD_ID, "preview_overlay"),
  ...
  private static void registerKey() {
    ...
    var previewToggle =
        KeyBindingHelper.registerKeyBinding(
            new KeyMapping("key.agma_standalone.preview_toggle", GLFW.GLFW_KEY_P, category));
    previewOverlay.toggleKey(previewToggle);
    var hologramToggle =
        KeyBindingHelper.registerKeyBinding(
            new KeyMapping("key.agma_standalone.hologram_toggle", GLFW.GLFW_KEY_O, category));
        ...
        client -> {
          ...
          while (hologramToggle.consumeClick()) {
            if (hologramLoaded) {
              ...
              clientMessage(
                  client, Component.translatable("chat.agma_standalone.hologram_unavailable"));
            } else {
              ...
              if (latest.isPresent()) {
                ...
                clientMessage(
                    client, Component.translatable("chat.agma_standalone.hologram_no_preview"));
              }
            }
          }
        });
  ...
  private static void onHologramLoad(boolean loaded, String reason) {
    ...
    clientMessage(Minecraft.getInstance(), Component.translatable(key));
  }

  private static void clientMessage(Minecraft client, Component message) {
    client.execute(
        () -> {
